...
;
            ...
            v2f vert(appdata v)
			{
                ...
                if (v.vertex.y >= _Change) {
					OUT.diff = lerp(_FirstC,_SecondC,(_Change /v.vertex.y) +.1f);
                }


				else if (v.vertex.y >= _Second && v.vertex.y < _Change) {
					OUT.diff = lerp(_ThirdC,_SecondC, (v.vertex.y / _Change));
